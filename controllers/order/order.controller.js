...
class order {
  static async createOrder(req, res) {
    try {
      ...
      const hotel = await HotelHelper.getHotel(
        body.hotel_id ? body.hotel_id : ""
      );
      if (!hotel) {
        throw HOTEL_NOT_FOUND;
      }

      const room = await RoomHelper.getRoom(
      ...
      const differenceInDays = await OrderHelper.calculateDays(
        body.check_in_date,
        body.check_out_date
      );
  ...
  static async makePayment(req, res) {
    try {
      const order = await OrderHelper.getOrders({
        matchQuery: { order_id: req.query.id },
      });
      ...
      const payment = await PaymentHelper.getPayment(
        order.orders[0].payment_id
      );
      ...
      if (!payment) {
        ...
        const orderUpdate = await OrderHelper.update({
          order_id: order.orders[0].order_id,
